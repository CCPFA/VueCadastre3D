...
function GuiTools(domId, view, w) {
    if (view) {
        ...
        this.colorGui = this.gui.addFolder('Couches');
        this.elevationGui = this.gui.addFolder('Modèles de terrain');
...
GuiTools.prototype.addElevationLayerGUI = function addElevationLayerGUI(layer) {
    ...
    folder.add({ scale: layer.scale }, 'scale').min(1.0).max(100.0).onChange((function updateScale(value) {
        layer.scale = value;
        this.view.notifyChange(layer);
    }).bind(this));
};
